# xui-uClibc v0.9.30.2		[ since v0.9.15, c.2002-10-14 ]
...
${CUI_XUI_UCLIBC_INSTALLED}:
	...
	( cd ${EXTTEMP}/${CUI_XUI_UCLIBC_TEMP} || exit 1 ;\
		case ${PKG_VER} in \
		0.9.30.2) \
			CDPATH='' make PREFIX="${CTI_ROOT}/usr/${TARGET_SPEC}/usr/" DEVEL_PREFIX='' install || exit 1 \
		;; \
		*) \
			make PREFIX="${CTI_ROOT}/usr/${TARGET_SPEC}/usr/" DEVEL_PREFIX='' install || exit 1 \
		;; \
		esac \
	)
