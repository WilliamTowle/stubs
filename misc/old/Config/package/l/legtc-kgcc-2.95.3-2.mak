# legtc-kgcc v2.95.3-2		[ since v2.7.2.3, c.2002-10-14 ]
...
CTI_KGCC_TARGET_MIN_SPEC:=$(shell echo ${TARGET_MIN_SPEC} | sed 's/uclibc$$/gnu/')
	...
	( cd ${EXTTEMP}/${CTI_KGCC_TEMP} || exit 1 ;\
		CC=${NATIVE_SPEC}-gcc \
		  ...
		  CFLAGS='-O2' \
		    ./configure -v \
			--prefix=${CTI_ROOT}/usr \
			--host=${CTI_KGCC_NATIVE_SPEC} \
			--build=${CTI_KGCC_NATIVE_SPEC} \
			--target=${CTI_KGCC_TARGET_MIN_SPEC} \
			--program-transform-cross-name='s,^,'${CTI_KGCC_TARGET_MIN_SPEC}'-,' \
			--enable-languages=c \
			--without-headers \
			--with-newlib \
			--enable-shared \
		...
		cat gcc/Makefile.OLD \
			| sed '/^GCC_CROSS_NAME/ s/`.*`/'${CTI_KGCC_TARGET_MIN_SPEC}'-gcc/' \
			> gcc/Makefile || exit 1 \
	)
...
CTI_KGCC_BUILT=${EXTTEMP}/${CTI_KGCC_TEMP}/libiberty/libiberty.a
...
CTI_KGCC_INSTALLED=${CTI_ROOT}/usr/bin/${CTI_KGCC_TARGET_MIN_SPEC}-gcc
...
${CTI_KGCC_INSTALLED}: ${CTI_ROOT}
	echo "*** $@ (INSTALLED) ***"
	( cd ${EXTTEMP}/${CTI_KGCC_TEMP} || exit 1 ;\
		GCC_CROSS_NAME=${CTI_KGCC_TARGET_MIN_SPEC}'-gcc' make install-gcc || exit 1 \
	)
