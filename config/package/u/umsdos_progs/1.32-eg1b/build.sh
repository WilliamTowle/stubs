...
#		echo "$0: CONFIGURE: Configuration not determined" 1>&2
	...
	if [ ! -r `echo ${FR_CROSS_CC} | sed 's/gcc$/g++/'` ] ; then
		echo "$0: CONFIGURE: No C++ compiler/wrapper" 1>&2
		exit 1
	elif [ ! -r ${FR_TH_ROOT}/usr/bin/g++ -a ! -r /usr/bin/g++ ] ; then
		echo "$0: CONFIGURE: C++ wrapper does not point to C++ compiler" 1>&2
		exit 1
	fi
