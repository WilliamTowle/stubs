...
# | sed '/^COPT/ s%=%= -nostdinc -I'${TCTREE}/${FR_UCPATH}'/include -I'${TCTREE}/${FR_UCPATH}'/include/ncurses -I'${GCCINCDIR}' -I'${TCTREE}'/usr/include %' \
	for MF in `find ./ -name Makefile` ; do
		[ -r ${MF}.OLD ] || mv ${MF} ${MF}.OLD || exit 1
		cat ${MF}.OLD \
			| sed '/^COPT/ s/-g/ /' \
			| sed '/^prefix/ s%=.*%= ${DESTDIR}/usr/local%' \
			| sed '/chown / s/	/#	/' \
			> ${MF} || exit 1
	done
...
# INSTALL...
	#mkdir -p ${INSTTEMP}/etc/lxt-conf || exit 1
