...
#		echo "$0: CONFIGURE: Configuration not determined" 1>&2
	...
	if [ ! -r ${FR_LIBCDIR}/include/id3tag.h ] ; then
		echo "$0: Failed: No id3tag.h?" 1>&2
		exit 1
	fi

	if [ ! -r ${FR_LIBCDIR}/include/mad.h ] ; then
		echo "$0: Failed: No mad.h?" 1>&2
		exit 1
	fi

# --build=`uname -m` --host=${TARGET_CPU} --target=${TARGET_CPU}
# CC=${FR_HOST_CC}
# CC_FOR_BUILD=${FR_CROSS_CC}

	CC=${FR_CROSS_CC} \
	  CPPFLAGS=-I${FR_LIBCDIR}/include \
