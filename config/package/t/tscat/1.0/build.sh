...
EOF
		...
		cat Makefile.OLD \
			| sed '/^	/	s/(BIN_DIR)/{DESTDIR}$(BIN_DIR)/' \
			| sed '/^	/	s/(MAN_DIR)/{DESTDIR}$(MAN_DIR)/' \
			>> GNUmakefile || exit 1

		[ -r tscat.c.OLD ] || mv tscat.c tscat.c.OLD || exit 1
		cat tscat.c.OLD \
			| sed '/buf\[0\]/		s%^%/* %' \
			| sed '/buf\[0\]/		s%$% */%' \
			| sed '/struct tm/	s/$/buf[0]= '"'"'['"'"';/' \
			| sed '/  struct timeval/	s%^%/* %' \
			| sed '/  struct timeval/	s%$% */%' \
			| sed '/int need_ts/		s%^%/* %' \
			| sed '/int need_ts/		s%;%; */%' \
			| sed '/char.*BUFSIZE/	s/^/int need_ts= 1; struct timeval start_tv, previous_tv, now_tv;/' \
			> tscat.c || exit 1
	;;
