...
#		echo "$0: CONFIGURE: Configuration not determined" 1>&2
	...
	if [ ! -d ${FR_KERNSRC} ] ; then
		echo "$0: Confused: No KTREE ${FR_KERNSRC}" 1>&2
		exit 1
	fi

	if [ ! -r ${FR_TH_ROOT}/usr/bin/which ] ; then
		echo "$0: No 'which' in toolchain" 1>&2
		exit 1
	fi
...
# BUILD...
	make CROSS=`echo ${FR_KERNEL_CC} | sed 's/gcc$//'` \
		HOSTCC=${FR_HOST_CC} \
...
# INSTALL...
	...
	( cd ${FR_TC_ROOT}/usr/bin \
		&& ln -sf ../${FR_TARGET_DEFN}/bin/${TARGET_CPU}-uclibc-gcc ${FR_TARGET_DEFN}-gcc \
		&& ln -sf ../${FR_TARGET_DEFN}/bin/${TARGET_CPU}-uclibc-g++ ${FR_TARGET_DEFN}-g++ \
	) || exit 1

#	( cd ${FR_TC_ROOT}/usr/${FR_TARGET}/bin || exit 1
#		for EXE in ar as nm objdump ranlib strip ; do
##			ln -sf ${FR_TC_ROOT}/usr/${TARGET_CPU}-cross-linux/bin/${EXE} ./
#			ln -sf ${FR_TC_ROOT}/usr/${TARGET_CPU}-kernel-linux/bin/${EXE} ${TARGET_CPU}-uclibc-${EXE}
#		done
#		ln -sf ${FR_TC_ROOT}/usr/bin/cpp ./
#	) || exit 1
}
