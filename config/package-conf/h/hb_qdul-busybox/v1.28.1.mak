...
## [2017-07-27] v1.27.1: CONFIG_FEATURE_SH_IS_ASH obsolete; removed
## [2017-07-27] v1.27.1: align PSTM_X86 assembly with snapshot
## [2017-07-27] v1.27.1: disable CONFIG_FALLOCATE - uClibc not compatible
...
${CUI_BUSYBOX_CONFIGURED}:
	( cd source/busybox-${PKGVER} || exit 1 ;\
		...
		case ${PKGVER} in \
		1.27.1) \
			[ -r networking/tls_pstm_mul_comba.c.OLD ] || mv networking/tls_pstm_mul_comba.c networking/tls_pstm_mul_comba.c.OLD ;\
			cat networking/tls_pstm_mul_comba.c.OLD \
				| sed '/(PSTM_X86)/,/(PSTM_X86_64)/ s/"=r"/"=rm"/g' \
				> networking/tls_pstm_mul_comba.c ;\
			[ -r networking/tls_pstm_sqr_comba.c.OLD ] || mv networking/tls_pstm_sqr_comba.c networking/tls_pstm_sqr_comba.c.OLD ;\
			cat networking/tls_pstm_sqr_comba.c.OLD \
				| sed '/(PSTM_X86)/,/(PSTM_X86_64)/ s/"=r"/"=rm"/g' \
		...
		(	echo 'CONFIG_PREFIX="'${INSTTEMP}'"' ;\
			...
			echo '# CONFIG_FALLOCATE is not set' ;\
