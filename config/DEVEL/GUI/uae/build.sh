...
# BUILD...
	make -C src/tools CC=${FR_HOST_CC} build68k || exit 1
	make -C src/tools CC=${FR_HOST_CC} cpuopti || exit 1
	make -C src/tools CC=${FR_HOST_CC} genblitter || exit 1
	make -C src/tools CC=${FR_HOST_CC} gencpu || exit 1
...
# INSTALL...
	...
	cp uae readdisk ${INSTTEMP}/usr/local/bin || exit 1
}
