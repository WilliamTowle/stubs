...
do_configure()
{
	...
	*)
	...
	case ${PHASE}-${FR_TARGET_DEFN}-${PKGVER} in
	dc-i386-linux-uclibc-4.5.1)
		#for SF in freadahead.c fseterr.c freading.c fseeko.c ; do
		for SF in freadahead.c freading.c fseeko.c ; do
			...
			cat gnulib/lib/${SF}.OLD \
				| sed '/ifdef __STDIO_BUFFERS/ s%ifdef __STDIO_BUFFERS%if 0 /* def __STDIO_BUFFERS */%' \
				| sed 's/__modeflags/modeflags/g' \
